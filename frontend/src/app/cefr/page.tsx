...
import { Play, Clock, BookOpen, Mic, PenLine, Search, FileText } from 'lucide-react';
...
export default function CefrPage() {
  ...
  const fetchMocks = async () => {
    try {
      const { data } = await api.get('/api/cefr/student/mocks');
      setMocks(data);
    } catch (error) {
  ...
  const handleStartExam = async (mockId: string) => {
    try {
      await api.post(`/cefr/student/mocks/${mockId}/start`);
      router.push(`/cefr/${mockId}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] flex">
      ...
      <main className="flex-1 p-6 lg:p-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">CEFR Mock Tests</h1>
          <p className="text-gray-400">Cambridge B2 First, C1 Advanced style mock tests</p>
        </div>
        ...
        {mocks.length === 0 ? (
          <div className="glass-dark rounded-2xl p-16 text-center">
            ...
            <p className="text-gray-400">
              {searchQuery ? 'Qidiruvni o\'zgartiring' : 'Hozircha CEFR mock testlar yo\'q'}
            </p>
          </div>
        ) : (
              ...
              <div key={mock.id} className="glass-dark rounded-2xl p-6 hover:border-primary-500/50 transition">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="flex items-center gap-2 text-gray-400 text-sm">
                    <Clock size={16} />
                    <span>{mock.listening?.duration || 40}d Listening</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-400 text-sm">
                    <BookOpen size={16} />
                  ...
                  <div className="flex items-center gap-2 text-gray-400 text-sm">
                    <PenLine size={16} />
                    <span>{mock.writing?.duration || 80}d Writing</span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-400 text-sm">
                    <Mic size={16} />
                  ...
                  <div className="mb-4 p-3 bg-white/5 rounded-lg">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-white font-medium">
                        Level: {mock.latestAttempt.cefrLevel || '-'}
                      </span>
